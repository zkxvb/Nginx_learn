...
//格式的用法，请参见4.4节中的表4-7
    ...
    ngx_int_t queryLineLen = backendQueryLine.len + r->args.len - 2;            //这里减去2的原因是：%V是格式转换符，在将r结构体中的url参数与请求行合并到一块时会被url替换掉，所以要减去%V的长度2
...
//降低内存泄漏的可能
    ...
    //作用相当于snprintf，只是它支持4.4节中的表4-7列出的所有转换格式，在这里构建出完整的请求
...
//之后  再收到的新字符流将由mytest_upstream_process_header解析
    u->process_header = mytest_upstream_process_header;

    //如果本次  收到的字符流除了http响应行外，还有多余的字符，
...
//方法，它用于解析http头部
        ...
        //返回NGX_OK表示解析出一行http头部，然后将解析出的http头部添加到headers_in.headers链表中
        if (rc == NGX_OK)
        {
